...
// ⚠️ 아래 두 표는 손으로 적지 말 것 — TourAPI areaCode2 응답 그대로다.
//    (과거 임의 작성본은 코드가 전부 어긋나 양평군→화성시가 나오는 버그를 냈다)
const AREA_MAP = {
  '1':'서울', '2':'인천', '3':'대전', '4':'대구', '5':'광주',
  '6':'부산', '7':'울산', '8':'세종', '31':'경기도', '32':'강원도',
  '33':'충청북도', '34':'충청남도', '35':'경상북도', '36':'경상남도', '37':'전북',
  '38':'전라남도', '39':'제주도'
};
...
/* ─ 구/군 드롭다운 업데이트 ─ */
// 지역 select 를 AREA_MAP 하나로 채운다 — HTML에 코드를 손으로 박아두면
// AREA_MAP 과 어긋나 "전라북도 선택 → 경상북도 결과" 같은 버그가 난다(실제 발생).
function populateAreaOptions(selectId, placeholder) {
  const sel = document.getElementById(selectId);
  if (!sel) return;
  const keep = sel.value;
  const opts = Object.entries(AREA_MAP)
...
async function _fetchDomesticPage(append) {
  ...
  try {
    ...
    if (_dom.keyword) {
      ...
      if (_dom.sigunguCode) params.sigunguCode = _dom.sigunguCode;  // 키워드 검색에서도 시군구 적용
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initDates();
  loadExchangeRates();
  populateAreaOptions('sw-area', '');          // 지역 옵션은 AREA_MAP 이 단일 출처
  populateAreaOptions('dfArea', '전체 지역');
