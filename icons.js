/* ═══════════════════════════════════════════════════════════
   트립가이드 아이콘 세트
   Lucide 스타일 (24×24 그리드 / stroke 1.75 / round cap·join)
   Lucide는 ISC 라이선스 — 상업적 사용 자유, 출처표기 불필요.

   외부 CDN·아이콘 폰트 의존 0 — 인라인 SVG라 오프라인·CSP 안전.

   사용법
     1) HTML:  <i data-icon="plane"></i>            → 자동 치환
        크기:  <i data-icon="plane" data-size="24"></i>
     2) JS:    el.innerHTML = icon('heart', 20)
   ═══════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /** 아이콘 SVG 문자열 반환. 없는 이름이면 빈 문자열. */
  function icon(name, size, cls) {
    var d = ICONS[name];
    if (!d) return '';
    size = size || 20;
    var filled = /fill="currentColor"/.test(d);
    return '<svg xmlns="' + NS + '" class="ico' + (cls ? ' ' + cls : '') +
      '" width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" ' +
      'stroke="currentColor" stroke-width="' + (filled ? 0 : 1.75) + '" ' +
      'stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">' +
      d + '</svg>';
  }

  /** [data-icon] 요소를 전부 SVG로 치환 (중복 실행 안전) */
  function hydrateIcons(root) {
    root = root || document;
    var nodes = root.querySelectorAll('[data-icon]:not([data-icon-done])');
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      var svg = icon(el.getAttribute('data-icon'), Number(el.getAttribute('data-size')) || 20);
